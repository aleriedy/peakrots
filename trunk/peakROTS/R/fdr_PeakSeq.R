...
fdr <- function(selected.readlength, selected.maxgap, selected.wsize, selected.wperc){
...
par1 <- selected.readlength
par2 <- selected.maxgap
par3 <- selected.wsize
par4 <- selected.wperc

#outputNameO <- "MacspeaksOriginal"
outputNameO <- "PeakseqpeaksOriginal"
#outputNameR <- "MacspeaksRandom"

outputNameR <- "PeakseqpeaksRandom"
...
peaksO <- parsePeakSeqResults(file.path(path.peaks, paste(outputNameO,
	"_READLENGTH", readlength[[selected.readlength]],
	"_MAXGAP", maxgap[[selected.maxgap]],
	"_WSIZE", wsize[[selected.wsize]],
	"_WPERC", wperc[[selected.wperc]],
	"_ORIG_FINAL", sep="")))
...
for(b in 1:bootstrap.count) {
	...
	peaksAR <- parsePeakSeqResults(file.path(path.peaks, paste(outputNameR,
		"_READLENGTH", readlength[[selected.readlength]],
		"_MAXGAP", maxgap[[selected.maxgap]],
		"_WSIZE", wsize[[selected.wsize]],
		"_WPERC", wperc[[selected.wperc]],
    ...
    FDR.A[,b]<- calculateFDR(observed=peaksO[,"-10log10q_value"], permuted=peaksAR[,"-10log10q_value"])
    FDR.B[,b]<- calculateFDR(observed=peaksO[,"-10log10q_value"], permuted=peaksBR[,"-10log10q_value"])
